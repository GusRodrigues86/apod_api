...
void run() {
  ...
  auto client = std::make_shared<RemoteController>();
  OATPP_COMPONENT(std::shared_ptr<RemoteApiClient>, apiClient);
  oatpp::web::server::api::Endpoints docEndpoints;
  docEndpoints.append(router->addController(client)->getEndpoints());

  /* Create RemoteClient and add all of its endpoints to router */
  router->addController(client);
  ...
  OATPP_LOGI("ApodApp", "Server running on port %s",
